...
public class LngLat {
     ...
     */
    public boolean inCentralArea(String baseUrl) {
        LngLat[] markers = CentralClient.getINSTANCE(baseUrl).getCentralMarkers();
    ...
    public LngLat nextPosition(Direction d) {
        if (d == Direction.Hover){return this;}
        double newLong = this.lng + Math.sin(d.angle()) * LngLat.MOVE_SIZE;
        double newLat = this.lat + Math.cos(d.angle()) * LngLat.MOVE_SIZE;
